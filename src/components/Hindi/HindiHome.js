...
import data from '../../data/hindi.json'
...
const HindiHome = () => {
    ...
    const { formatHindiData} = useContext(CityContext);
    
    const [search, setSearch] = useState();
    const [searchResults, setSearchResults] = useState(formatHindiData());
    
    const onSearchChange = (e) => {
        if(e.target.value.length < 1){
            setSearch(null)
            setSearchResults(formatHindiData())
        }else {
            setSearch(e.target.value)
            const results = formatHindiData().filter((val) => {
                        ...
                        {   searchResults.map(city => (
                                <Col xs={12} sm={6} md={4} lg={3} xl={3} className="mx-auto">
                                    <div className="product-wrap d-flex justify-content-between flex-column align-items-center">
                                        <h2>{city.name}</h2>
                                        <Link to={{pathname: `/details/hindi/${city.cityID}`}}>
